...
MEDIUM_PATTERNS = [
    ...
    (r'McAfee.*SiteList.*\.xml.*[A-Z]:\\',           'McAfee SiteList.xml found - may contain credentials'),
    ...
    (r'(password|passwd)\s*[=:]\s*(?!Can|Not|Req|None|Yes|No|True|False|Last|Age|Min|Max|Length|History|Prop|Empty|Complex)\S{6,}', 'Possible plaintext password in config/file'),
...
INFO_PATTERNS = [
    ...
    (r'OS Name\s*:.*Windows|ProductName\s*:.*Windows|Windows \d{2} (Pro|Enterprise|Server)', 'OS version identified'),
